...
if __name__ == '__main__':
    ...
    train_pos_seqs = np.array(read_fasta(Data_dir + '/train_p.txt'))

    train_neg_seqs = np.array(read_fasta(Data_dir + '/train_n.txt'))
    # one-hot NCP-D
    ...
    train_label = np.array([1] * 15 + [0] * 15).astype(np.float32)
    train_label = to_categorical(train_label, num_classes=2)

    #
    # Read the testing set
    test_pos_seqs = np.array(read_fasta(Data_dir + '/test_p.txt'))
    test_neg_seqs = np.array(read_fasta(Data_dir + '/test_n.txt'))
    ...
    test_label = np.array([1] * 15 + [0] * 15).astype(np.float32)
